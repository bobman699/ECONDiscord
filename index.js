...
// TOP.GG API ***DISABLE WHEN BETA TESTING***

const { AutoPoster } = require("topgg-autoposter");

try{
    if(!process.env.STATUS || process.env.STATUS !== "DEV"){
...
process.on('uncaughtException', async function(err) {
    ...
    const EmbedError = new EmbedBuilder()
        .setColor('#ff675c')
        .setTitle('ERROR. ERROR. ERROR.')
        .setTimestamp()
        .setDescription(err.message)

    const discHook = new WebhookClient({url: "https://discord.com/api/webhooks/1134576638101815346/QujlY0XVc_9O_fAQKw-VibKTduidBCfUV67_xZbzpbAZpEr4B3YnTSUWFoblXaDVmED7"})

    try{
       await discHook.send({
            content: '',
            username: 'Watch Tower',
            embeds: [EmbedError],
        });
    }catch(e){
        console.warn(e)
...
try {
    if(!process.env.STATUS || process.env.STATUS !== "DEV"){
        request('https://uptime.betterstack.com/api/v1/heartbeat/aZgyLiUKXs5VKjR5q7NVJArg').then((promise)=>{
            console.log('Posted Better Stack Checked-In')
        })
    }

    }catch(e){
        console.warn('<<< INCIDENT >>>UNABLE TO CHECK IN')
        console.warn(e)
}
setInterval(async ()=>{
    if(!process.env.STATUS || process.env.STATUS !== "DEV"){
        try {
            ...
            await request('https://uptime.betterstack.com/api/v1/heartbeat/aZgyLiUKXs5VKjR5q7NVJArg').then((promise)=>{
                console.log('Posted Better Stack Checked-In')
            })
        }catch(e){
            console.warn('<<< INCIDENT >>>UNABLE TO CHECK IN')
